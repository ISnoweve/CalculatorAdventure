...
namespace _Main.MobSys.View.UI_Mob.Control
{
    public class MobViewControl : SingletonMonoBehaviour<MobViewControl>
    {
        ...
        private void SubscribeEvent()
        {
            ...
            GlobalMessagePipe.GetSubscriber<Calculate_UpdateMobQuestionNumber>().Subscribe(UpdateMobQuestionNumber).AddTo(bag);
            GlobalMessagePipe.GetSubscriber<Calculate_MobDefeated>().Subscribe(UpdateMobDefeated).AddTo(bag);
            GlobalMessagePipe.GetSubscriber<MobTurn_UpdateBehaviourNumber>().Subscribe(UpdateMobBehaviourCountDown).AddTo(bag);
            GlobalMessagePipe.GetSubscriber<MobTurn_SetMobNewBehaviour>().Subscribe(SetNewMobBehaviour).AddTo(bag);
            GlobalMessagePipe.GetSubscriber<Event_AtkS_Recover_TakeCalculatorButtonsMultiply>().Subscribe(RecoverByButtonsMultiply).AddTo(bag);
            GlobalMessagePipe.GetSubscriber<Event_AtkS_Recover_TakeCalculatorButtonsAddOrSubtract>().Subscribe(RecoverByButtonsAddOrSubtract).AddTo(bag);
            GlobalMessagePipe.GetSubscriber<Event_AtkS_Recover_Multiply>().Subscribe(RecoverByMultiply).AddTo(bag);
            GlobalMessagePipe.GetSubscriber<Event_AtkS_Recover_AddOrSubtract>().Subscribe(RecoverByAddOrSubtract).AddTo(bag);
        ...
        private void UpdateMobQuestionNumber(Calculate_UpdateMobQuestionNumber data)
        {
            ...
            FinishedUpdateNewNumber finishedUpdateNewNumber = new FinishedUpdateNewNumber();
        ...
        {
            int index = 0;
            foreach (var VARIABLE in data.TakeButtons)
            {
                index += VARIABLE.CurrentValue;
            }
            uiMobQuestionNumber.UpdateNewQuestionNumber(data.MobNewQuestionNumber);
        ...
        private IEnumerator Stay()
        {
            ...
            FinishedUpdateBehaviourCountDown finishedUpdateBehaviourCountDown = new FinishedUpdateBehaviourCountDown();
            GlobalMessagePipe.GetPublisher<FinishedUpdateBehaviourCountDown>().Publish(finishedUpdateBehaviourCountDown);
        }
    }
}